public class CoffeeVendingMachineFrame2 extends JFrame {
   // 커피 시뮬레이션 패널
   private CoffeeMachinePanel coffeeMachinePanel = new CoffeeMachinePanel();

   public CoffeeVendingMachineFrame2() {
   ...
   class CoffeeMachinePanel extends JPanel {
      ...
      public CoffeeMachinePanel() {
                              // 컴포넌트의 위치를 마음대로 지정할 수 있도록 null로 설정
         
         for(int i=0; i<boxes.length; i++) {
                                             // 재료통을 표시하는 레이블 컴포넌트 생성
                                             // 레이블의 위치 설정
                                              // 레이블의 크기 설정
         ...
         if(cmd.equals("Black Coffee")) { // 눌러진 버튼이 "Black Coffee" 이면
            ...
            if(boxes[CUP].isEmpty() || boxes[COFFEE].isEmpty() || boxes[WATER].isEmpty() ||
               ...
               return;
                                  ...
                                  // 컵을 초기 상태와 같이 가득 채운다.
                                  // 커피를 초기 상태와 같이 가득 채운다.
                                  // 물을 초기 상태와 같이 가득 채운다.
                                  // 설탕을 초기 상태와 같이 가득 채운다.   
                                  // 크림을 초기 상태와 같이 가득 채운다.
                                  // boxes[]의 레이블 컴포넌트에 변화가 생겼으므로 부모에게 다시 그리도록 한다.
            return;
         }
   ...
   static public void main(String [] args) {
      new CoffeeVendingMachineFrame2();
   }
}
